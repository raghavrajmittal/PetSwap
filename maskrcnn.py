...
def get_masks(fnames):
    ...
    imgs = []
    for fname in fnames:
        imgs.append(skimage.io.imread(fname))
    # image = skimage.io.imread(os.path.join(IMAGE_DIR, random.choice(file_names)))
    results = model.detect(imgs, verbose=0)  # Run detection

    # filter out non chosen classes
    chosen_classes = [16, 17]  # class indices for cat (16), dog (17)
    for r in results:
